...
"no `pip install`" rule (SETLIST_ADMIN_USB_SPECIFICATION.md section 2).

Entry point for `setlist-admin.service` (only ever started by
usb-tether-watchdog.service while a phone is tethered -- section 4c).
...
systemd level (section 9), not here.
"""
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _parse_upload_filename(self):
        ...
        without buffering the whole thing here first (section 9)."""
