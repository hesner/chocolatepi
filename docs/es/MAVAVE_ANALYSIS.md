...
**Impacto en la capacidad máxima:** con D reservado para STOP en cada grupo (decisión ya aprobada, sección 4.5), la capacidad real es `8 grupos × 3 canciones reales (A,B,C) = 24 canciones máximo`, no 96 como se había calculado antes. Esto es insuficiente para el repertorio real completo de la banda (~25-30 canciones).

**Decidido (aceptado, no es un bloqueante):** 24 canciones por colección `Set` activa es el techo real de este controlador específico — no es un bug que resolver. La arquitectura por capas de este proyecto (sección 3 de `MASTER_SPECIFICATION.md`) ya existe específicamente para que un futuro controlador MIDI con más bancos/grupos pueda elevar este techo agregando solo un `Adapter` nuevo, con cero cambios al `Mapper` o al `Core` (ver la fila "Futuro", sección 2). Hasta entonces, 24 es el límite aceptado para este hardware.
