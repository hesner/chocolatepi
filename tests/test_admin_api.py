...
mount to remount in a unit test.
...
class TestReboot(ApiTestCase):
    @patch("admin.api.subprocess.run")
    def test_reboot_calls_systemctl_reboot(self, mock_run):
        mock_run.return_value = MagicMock(returncode=0)

        self.api.reboot()

        mock_run.assert_called_once_with(
            ["sudo", "systemctl", "reboot"], capture_output=True, timeout=5,
        )

    @patch("admin.api.subprocess.run")
    def test_reboot_raises_api_error_if_it_cannot_run(self, mock_run):
        mock_run.side_effect = FileNotFoundError()
