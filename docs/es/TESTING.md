...
### Hallazgo de seguimiento (2026-09-05, sesión en vivo de desarrollo del Core)

Con el mismo cargador de 5V/2.5A ya validado arriba, se observó un segundo evento de subvoltaje aislado vía `dmesg` (`hwmon1: Undervoltage detected!` ... `Voltage normalised` ~4s después) cerca de las 3h40m de una sola sesión inusualmente pesada: ambos carriles de `mpv` (video + solo-audio) corriendo continuamente, combinado con reinicios frecuentes de `main.py`, una codificación con `ffmpeg` (conversión a MP3 + generación del fallback-standby), y ciclos repetidos de montaje/desmontaje del USB, todo superponiéndose en varios momentos — una carga combinada mucho más pesada y sostenida que un show real, que no reinicia procesos a mitad de la presentación. `vcgencmd get_throttled` justo después no mostró subvoltaje activo en ese momento (`0x50000` = solo bits históricos), y el sistema se mantuvo totalmente responsivo durante todo el proceso — sin cuelgue, sin MIDI perdido, sin corrupción de audio/video observada. No se trata como un problema nuevo de hardware que requiera arreglo; se anota acá como evidencia a vigilar durante una eventual prueba de duración completa del show con el contenido y setup de hardware final (no una sesión de desarrollo/pruebas con tanto reinicio de procesos).
...
### Prueba de frecuencia de refresco en el TV real (2026-09-05, cierra el seguimiento de arriba)

Repetida con el TV real del show (se trasladó la Pi hasta allá, ciclo de energía completo desde apagado -- también, de paso, la primera validación real de que el arranque automático de `systemd`, sección de `systemd/` más abajo, funciona de punta a punta desde un apagado genuino, no solo un `systemctl restart`).

- **La hipótesis era correcta:** `sudo modetest -c` confirmó que este TV soporta nativamente 1920x1080 a 24.00, 23.98, 25.00, 29.97, 30.00, y 50.00Hz (`mpv --drm-mode=help` lista el mismo conjunto) -- ninguno de los cuales ofrecía el monitor de PC de la prueba original.
- **Pero forzar el modo exacto no valió la pena.** El video de standby real es a 25fps; forzar `--drm-mode=1920x1080@25` (contra el `preferred` por defecto de mpv, que negocia 60Hz en este TV) se comparó A/B directamente contra el default, mismo contenido, mismo TV:
  - `drop-frame-count` ya era `0` a 60Hz -- no se estaba perdiendo ningún cuadro de entrada, así que no había nada que el modo exacto arreglara en ese frente.
  - El uso de CPU del carril de video fue estadísticamente igual en ambos casos (~100-105%, un core completo) -- confirmado probando también sin `--force-window` y sin la tasa de muestreo de audio fija, descartándolos como causa. Esto parece ser inherente a renderizar este contenido vía `--vo=gpu`/`v4l2m2m-copy` en este SoC, no relacionado con la coincidencia de frecuencia de refresco. No es un problema bloqueante por sí solo -- la Pi 2 tiene 4 cores, y esto nunca causó un cuadro perdido, un MIDI perdido, ni un glitch de audio en ninguna prueba de hoy -- pero es una característica real y abierta del pipeline de video actual, no explicada más a fondo acá.
  - **La calidad visual fue peor a 25Hz**, confirmado por el usuario viendo ambos lado a lado en el TV real.
- **Decisión:** se mantuvo `--drm-mode=preferred` (el default) en producción. Se dejó configurable vía `--drm-mode` en `src/main.py` / `Player`, por si una futura combinación de pantalla o contenido se beneficia donde esta, mediblemente, no lo hizo.

La sección 4.0 queda cerrada: sincronización audio-video validada, y la línea de investigación del desajuste de frecuencia de refresco del monitor de PC original se siguió hasta el TV real y una conclusión real, basada en evidencia (no un arreglo, sino un no-problema confirmado) en vez de quedar abierta.
